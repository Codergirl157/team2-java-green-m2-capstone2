...
AND reservation.end_date < '2021-10-31' 
   ...
   WHERE space.venue_id = 5 
    AND space.id NOT IN (SELECT space.id FROM space 
LEFT JOIN reservation ON space.id = reservation.space_id 
   WHERE ('2021-10-17' <= reservation.end_date 
                AND '2021-10-25' >= reservation.start_date) 
                OR EXTRACT(MONTH FROM CAST('2021-10-17' AS DATE)) < space.open_from 
                OR EXTRACT(MONTH FROM CAST('2021-10-25' AS DATE)) > space.open_to 
                OR space.max_occupancy < 50) 
